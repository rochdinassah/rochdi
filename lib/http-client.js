...
HttpClient.prototype.findCipher = function (url, opts = {}) {
  return new Promise(async resolve => {
    ...
    const stopOnFound = opts.stopOnFound;
    
    const results = [];
    const ciphers = [void 0, ...tls.getCiphers().map(cipher => cipher.toUpperCase())];
    for (const cipher of ciphers) {
      try {
        const { statusCode, data } = await this._request(method, url, { cipher, headers, retryOnError: false });
        if (statusCodes.includes(statusCode))
          continue;
        results.push({ cipher, statusCode, data });
        break;
      } catch {}
    }
