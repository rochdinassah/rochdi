...
Discord.prototype.onGuildRoleCreate = function (data, isUpdate) {
  ...
  log('role "%s" %s', role.name, isUpdate ? 'updated' : 'created');
};
...
Discord.prototype.onChannelCreate = function (data, isUpdate) {
  ...
  const channel = new Channel(this, guild, data);

  channels.set(channel.id, channel);
  log('channel "%s" %s', channel.name, isUpdate ? 'updated' : 'created');
};
...
class Guild {
  constructor(manager, opts = {}) {
    ...
    for (const role of opts.roles) {
      if (role.id === this.id)
        continue;
      this.roles.set(role.id, new Role(manager, this, role));
    }
    for (const channel of opts.channels) {
      this.channels.set(channel.id, new Channel(manager, this, channel));
    }
  }
  ...
  _createChannel(type, name, isPrivate) {
    const { id, manager, channels } = this;
    ...
    return manager.post('guilds/'+id+'/channels', body).then(res => {
      ...
      const channel = new Channel(manager, this, data);
      this.channels.set(channel.id, channel);
      
      return log('channel "%s" created, curr size:', channel.name, channels.size), channel;
    });
  }
...
class Role {
  constructor(manager, guild, opts = {}) {
    this.manager = manager;
    this.guild = guild;
...
class Channel {
  ...
  delete() {
    ...
    return manager.delete('channels/'+id).then(res => {
      ...
      if (200 !== statusCode)
        throw new Error('Channel.delete: request error, http('+statusCode+')');
