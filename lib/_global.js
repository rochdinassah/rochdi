...
global.parseHeaders = function (raw) {
  ...
  const headers = {}, m = randomString(16);
  match.filter(e => !/content-length/i.test(e[1])).forEach(e => headers[m+e[1]+m] = m+(/cookie/i.test(e[1]) ? '' : e[2])+m);
  return JSON.stringify(headers, null, 2).replaceAll('"'+m, '\'').replaceAll(m+'"', '\'');
};
