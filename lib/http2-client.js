...
Http2Client.prototype._request = function (method, url_string, opts = {}) {
  return new Promise((resolve, reject) => {
    ...
    this.ensureSession(authority, { cipher }).then(session => {
      ...
      stream.once('error', this.onError.bind(this, { resolve, reject }, arguments, session));
...
function onSessionConnect(session) {
  ...
  session.ping_interval_id = setInterval(pingsession.bind(session), this.ping_interval);
...
function pingsession() {
  this.ping(ping_buff, clearTimeout.bind(void 0, setTimeout(this.destroy.bind(this), 2**11)));
};