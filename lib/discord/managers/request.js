...
const ChannelComponent = require('../components/channel.js');
...
class ApiManager extends EventEmitter {
  constructor(manager) {
    ...
    const { logger, token, type } = manager;
  ...
  fetchMe(attempt = 1) {
    if (3 === attempt)
      throw new Error('ApiManager.fetchMe: unable to retrieve user infos');
    ...
    return this.get('users/@me', void 0, { 'authorization': 2 === attempt ? token : 'Bot '+token }).then(res => {
      ...
      this.manager.type = data.bot ? 'bot' : 'user';
      this.compileHeaders();
    });
  }
}
