...
function Discord(token, opts = {}) {
  ...
  this.channel_service = new RoleService(this);
  this.role_service = new RoleService(this);
...
Discord.prototype.onconnectionclose = function (code, buff) {
  ...
  this.ready = false;
  this.logger.warn('connection close, code: %d, data: %s', code, String(buff));
...
Discord.prototype.onHeartbeatInterval = function (data) {
  ...
  clearInterval(this._heartbeat_interval_id);
  this._heartbeat_interval_id = setInterval(this.heartbeat.bind(this), heartbeat_interval);
  this.heartbeat();
};
...
Discord.prototype.onHeartbeatRequest = function (data) {
  log(data);
  this.logger.error('recv heartbeat request');
  this.heartbeat();
};
...
Discord.prototype.respondInteraction = function (opts = {}) {
  ...
  return this.post(format('interactions/%s/%s/callback', id, token), body);
};
