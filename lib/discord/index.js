...
const RequestManager = require('./managers/request');
...
Discord.prototype.connect = function () {
  ...
  return new Promise(resolve => {
    this.fetchMe().then(() => {
      const { request_manager, type } = this;

      // request_manager.get('gateway'+('bot' === type ? '/bot' : '')).then(res => {
...
Discord.prototype.onconnectionclose = function (code, buff) {
  const { heartbeat_interval_id, state, session_id, seq, logger, request_manager } = this;
  ...
  if (1000 === code || 1001 === code)
    return request_manager.destroy();
...
Discord.prototype.respondInteraction = function (opts = {}) {
  ...
  return this.request_manager.post(format('interactions/%s/%s/callback', id, token), body);
};

Discord.prototype.fetchMe = function (attempt = 1) {
  if (3 === attempt)
    throw new Error('Discord.me: unable to retrieve user infos');
  
  const { logger, request_manager, token } = this;

  return request_manager.get('users/@me', void 0, { 'authorization': 2 === attempt ? token : 'Bot '+token }).then(res => {
    const { status_code, data } = res;
    ...
    this.type = bot ? 'bot' : 'user';

    request_manager.compileHeaders();
  });
};

Discord.prototype.createMessage = function (channel_id, content, embeds) {
  const { logger, request_manager } = this;
  return request_manager.post('/channels/'+channel_id+'/messages', { content, embeds, tts: !1 }).then(res => {
    const { status_code, data } = res;
...
Discord.prototype.deleteMessage = function (channel_id, message_id) {
  const { logger, request_manager } = this;
  return request_manager.delete('channels/'+channel_id+'/messages/'+message_id).then(res => {
    const { status_code, data } = res;
...
Discord.prototype.clearDM = function (conversation_id, offset = 0) {
  ...
  return this.request_manager.get(format('channels/%s/messages/search?author_id=%s&offset=%d', conversation_id, user_id, offset)).then(async res => {
    const { status_code, data } = res;
    // if (200 !== status_code)
    //   return false;
...
Discord.prototype.fetchCommands = function () {
  ...
  return this.request_manager.get(format('applications/%s/commands', application_id)).then(res => {
    const { status_code, data } = res;
...
Discord.prototype.createCommand = function (opts = {}) {
  ...
  return this.request_manager.post(format('applications/%s/commands', application_id), body).then(res => {
    const { status_code, data } = res;
...
Discord.prototype.deleteCommand = function (command_id) {
  ...
  return this.request_manager.get(format('applications/%s/commands/%s', application_id, command_id)).then(res => {
    const { status_code, data } = res;
