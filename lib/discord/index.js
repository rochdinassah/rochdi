...
Discord.prototype._authenticate = function () {
  ...
  const authTemplate = require('./templates/'+('bot' === type ? 'auth-bot' : 'auth'))
  authTemplate.token = token;
  authTemplate.properties.browser_user_agent = http2_client.user_agent;
...
Discord.prototype.create_message = function (channel_id, content, embeds) {
  const { logger } = this;
  const body = {
    content,
    embeds,
...
Discord.prototype.delete_message = function (channel_id, message_id) {
  return this.delete(format('channels/%s/messages/%s', channel_id, message_id)).then(res => {
    const { status_code, data } = res;
    if (204 !== status_code)
      return this.logger.warn('delete_message: request error, http(%d)', status_code), false;
...
Discord.prototype.clearDM = function (conversation_id, offset = 0) {
  ...
  return this.get(format('channels/%s/messages/search?author_id=%s&offset=%d', conversation_id, user_id, offset)).then(async res => {
    ...
    for (const message of messages) {
      ...
      await this.delete_message(channel_id, id);
