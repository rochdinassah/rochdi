...
class Discord extends EventEmitter {
  ...
  async onReadyMessage(data) {
    ...
    if (!user.bot)
      ...
      for (const guild of guilds)
        pending_guild_promises.push(new Promise(resolve => guild_manager.once('GuildObjectCreated::'+guild.id, resolve)));

    await Promise.all(pending_guild_promises);
  ...
  sendMessage(channel_id, content, embeds = []) {
    ...
    return api_manager.post('/channels/'+channel_id+'/messages', { content, embeds, tts: !1 }).then(res => {
      ...
      if (200 !== status_code)
        return void logger.warn('sendMessage: request error, http(%d)', status_code);
      return data.id;
    });
  }
