...
class Discord extends EventEmitter {
  ...
  onReadyMessage(data) {
    ...
    const { username, email } = this.user = user, promises = [];
    ...
    Promise.all(promises).then(() => {
      ...
      if (user.bot)
        logger.verbose('auth ok(bot) | username(%s), commands(%d)', username, command_manager.size);
      else
        logger.verbose('auth ok(user) | username(%s), email(%s)', username, email);
