...
class Openai extends EventEmitter {
  ...
  fetch_requirements() {
    ...
    return this.post(url, body, headers).then(res => {
      ...
      if (200 !== status_code)
        return logger.error('fetch_requirements: request error, http(%d)', status_code), false;
  ...
  send_message(content) {
    ...
    return this.get_requirements().then(({ proof, token }) => {
      ...
      return this.post(url, body, headers).then(res => {
        ...
        if (200 !== status_code)
          return logger.warn('send_message: request error, http(%d)', status_code), false;

        const matches = Array.from(/data\: (.*})/g[Symbol.matchAll](data));

        if (!matches.length)
          return logger.warn('send_message: matches array is empty'), false;
